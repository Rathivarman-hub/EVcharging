...
export const addStation = async (req, res) => {
  try {
    ...
    // Generate slots — one full set of time windows per pump
    const baseTimeSlots = [
    ...
    const numPumps = Math.max(1, totalSlots); // totalSlots = number of pumps
    const slotDocs = [];

    for (let pump = 1; pump <= numPumps; pump++) {
    ...
    // Update station with all slot references
